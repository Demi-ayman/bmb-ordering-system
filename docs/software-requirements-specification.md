...
| `GET` | `/api/v1/admin/orders` | Administrator | Retrieve all customers' orders |
...
- CORS shall be restricted to approved frontend origins.
...
- Order-list endpoints shall support pagination.
...
12. Automated tests cover authentication, ownership, deletion counting, and ban expiry.
...
## 11. Open Questions for the Stakeholder

1. Should "same day" use UTC, Egypt time, server time, or the customer's timezone?
2. Can a banned customer continue deleting existing orders?
3. Should a deletion during an existing ban restart or extend the six-hour period?
4. What exact information must an order and order item contain?
5. Should deleted orders be visible to the customer in a history view?
6. Is an administrator role expected for the "retrieve all orders" function?
7. Is .NET 6 strictly mandatory despite being out of Microsoft support?
